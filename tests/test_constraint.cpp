...
TEST(PolarInequalityConstraintTest, InvalidParameters) {
    ...
    EXPECT_THROW(PolarInequalityConstraint(G, c, 1.0, 10.0, -0.1), std::invalid_argument);

    // Test with invalid bf_gamma (> 1)
    EXPECT_THROW(PolarInequalityConstraint(G, c, 1.0, 10.0, 1.5), std::invalid_argument);

    // Test with invalid bounds (lwr_bound >= upr_bound)
    EXPECT_THROW(PolarInequalityConstraint(G, c, 10.0, 5.0, 0.5), std::invalid_argument);
}
