...
Swarm::Swarm(int num_drones, int K, int n, float delta_t, Eigen::VectorXd p_min, Eigen::VectorXd p_max, float w_g_p, float w_g_v, float w_s, int kappa, float v_bar, float f_bar, std::vector<Eigen::VectorXd> initial_positions, std::vector<Eigen::MatrixXd> waypoints)
    : num_drones(num_drones), K(K)
{
    ...
    for (int i = 0; i < num_drones; ++i) {
        ...
        drones.emplace_back(Drone(K, n, delta_t, p_min, p_max, w_g_p, w_g_v, w_s, kappa, v_bar, f_bar, initial_positions[i], waypoints[i]));
